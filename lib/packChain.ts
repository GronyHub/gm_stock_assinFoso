...
// those dates came out as a real loss (gains don't count).
export function computeChainLossSummary(packDayRows: ItemDayRow[], singlesDayRows: ItemDayRow[], unitsPerPack: number, sheetPrice: number): { lossAmt: number; lossCount: number } {
  ...
  return { lossAmt: parseFloat(lossAmt.toFixed(2)), lossCount }
}
