...
// scratchpad), separate from the formal Company Laws content.
export async function ensurePageNotesTable() {
  ...
  await sql`CREATE UNIQUE INDEX IF NOT EXISTS page_notes_scope_kind_idx ON page_notes (scope_key, kind)`.catch(() => {})
}
