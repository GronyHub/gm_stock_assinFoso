...
const TOPICS: Topic[] = [
    ...
    body: (
      <div className="space-y-3">
        <Steps items={[
          ...
          <>Tap its <MiniIconBtn tone="blue">+</MiniIconBtn> button. A sheet opens with the item's name, selling price, cost price, and current stock.</>,
          <>Enter the quantity sold. If the item has a custom selling price for this sale, you can override the price too.</>,
          <>Confirm. The tap is recorded immediately and rolls into today’s sales receipt.</>,
        ]} />
    ...
    body: (
      <div className="space-y-3">
        ...
        <ul className="space-y-2 text-sm text-gray-700 list-disc list-inside">
          ...
          <li><strong>GMC</strong> (Grony Multimedia as Customer) — for stock the shop takes for its <strong>own internal use</strong>, not a real sale. It is excluded from revenue and profit, and it’s how the system explains stock that left the shelf without a walk-in sale (e.g. taking a pack of paper to print in-house). Tapping this shows a loud red warning banner across the top the whole time GMC is selected, precisely so it’s hard to leave on by accident.</li>
        </ul>
        ...
        <p className="text-sm text-gray-700">While GMC is selected, the grid also only shows items that already have some GMC history — a brand-new item can’t be GMC-taken for the very first time from here (it needs a normal restock/bill first).</p>
      </div>
    ),
  },
  {
    id: 'custom-quantity',
    title: 'Selling a quantity that isn’t a preset button',
    group: 'Live tab',
    keywords: 'custom quantity amount preset number type',
    body: (
      <p className="text-sm text-gray-700">Every tap sheet has a plain quantity field — type any number, it doesn’t have to match a suggested amount. There’s no separate "custom" mode; you always type the exact quantity being sold.</p>
    ),
  },
  {
    id: 'undo-a-tap',
    ...
    body: (
      <div className="space-y-3">
        <p className="text-sm text-gray-700">Tapped the wrong item, or the wrong quantity? Go to the <MiniPill text="Log" /> tab, find the tap in today’s list, and undo it there.</p>
        <Callout>Undoing doesn’t delete history — the tap stays visible in the Log, just crossed out and excluded from the day’s total, so there’s always a record of what was corrected and when.</Callout>
      </div>
    ),
  },
  {
    ...
    body: (
      <div className="space-y-3">
        ...
        <p className="text-sm text-gray-700">Tap the item and a count field appears right inside the sale sheet — you don’t need to switch to the Count tab to clear it. Enter what’s actually on the shelf and submit; the item drops off this list once it’s counted.</p>
      </div>
    ),
  },
  {
    ...
    body: (
      <div className="space-y-3">
        <p className="text-sm text-gray-700">If an item is overdue for a count, the system will refuse to record a new sale, live tap, or bill against it until it’s counted. This is deliberate: letting transactions pile up on stale stock numbers is exactly how stock records quietly drift away from what’s really on the shelf.</p>
        ...
        <Callout>Purchase Orders are not blocked by this — restocking an item is allowed even if it’s overdue for a count.</Callout>
      </div>
    ),
  },
  {
  ...
  },
  {
    ...
    body: (
      <div className="space-y-3">
        <p className="text-sm text-gray-700">Type into the search box to jump straight to an item by name. Selecting a result from the dropdown pins the grid to show <strong>only</strong> that item — handy on a small screen, or when you already know exactly what you’re looking for.</p>
  ...
  {
    id: 'edit-item',
    title: 'How to edit an item’s details',
    group: 'Item settings',
    ...
    body: (
      <div className="space-y-3">
        ...
        <p className="text-sm text-gray-700">Save applies immediately — you don’t need to leave the sheet.</p>
        <Callout>Deleting an item entirely isn’t done from Live Sale — that lives on the Items page, and only works once the item has no sales/bills/count history (otherwise it needs to be merged into another item instead).</Callout>
      </div>
    ),
  },
  {
    ...
    body: (
      <div className="space-y-3">
        <p className="text-sm text-gray-700">In the item’s Edit sheet, under Stock counts, the <strong>Count every</strong> dropdown sets how often this specific item should be counted:</p>
        <ul className="space-y-1 text-sm text-gray-700 list-disc list-inside">
          <li><strong>Automatic</strong> (default) — the system decides based on the item’s own history (sold via GMC → every 7 days; otherwise every 15 or 30 days depending on how steady its counts have been).</li>
        ...
        <p className="text-sm text-gray-700">The "Currently: …" line above the dropdown shows what the item is actually resolving to right now, so you can tell whether it’s already doing what you want before changing anything.</p>
      </div>
    ),
  },
  {
    ...
    body: (
      <div className="space-y-3">
        <p className="text-sm text-gray-700">For stock that’s genuinely gone for good (discontinued, off the market), tick <strong>Exclude from counts entirely</strong> in the item’s Edit sheet. You’ll need to give a reason (pick one, or choose "Other…" to type your own).</p>
        <Callout kind="warn">This only takes effect once the item’s stock is at exactly 0 — if it still shows stock, bring it to zero first (a count, or a sale/bill that clears it out), then exclude it. This stops an item that still has real stock on the shelf from silently disappearing from every count queue.</Callout>
      </div>
    ),
  },
  {
    ...
    body: (
      <p className="text-sm text-gray-700">Shows every tap you’ve made, grouped by date, most recent day first. Use it to double-check today’s sales against your cash total, review a past day, or undo a tap that wasn’t caught right away.</p>
    ),
  },
  {
    id: 'sales-tab',
    ...
    body: <p className="text-sm text-gray-700">The classic list of sales receipts — useful for named-customer sales that don’t belong on the walk-in tap grid.</p>,
  },
  {
    id: 'bills-tab',
    title: 'The Bills tab',
  ...
  },
  {
    ...
    body: (
      <div className="space-y-3">
        ...
        <p className="text-sm text-gray-700"><strong>Dormant</strong> means an item’s last two counts both came back at zero with no bill since — it’s dropped out of the regular count rotation until a new bill brings stock back in, so it doesn’t keep nagging for a count of nothing.</p>
      </div>
    ),
  },
  {
    ...
    body: (
      <div className="space-y-3">
        <p className="text-sm text-gray-700">This is the Count tab’s landing view — every count ever entered, newest day first, with: Item, Group, Qty counted, <strong>Exp</strong> (what the records expected), <strong>Loss/Gain</strong> (the difference — red for a loss, amber for a gain), Time, who counted it (By), Source, and Notes.</p>
  ...
  },
  {
    ...
    body: <p className="text-sm text-gray-700">The audit log of count-related actions — who counted, edited, or deleted what, and when. Use this to trace back a change that doesn’t match what you expected in Count Records.</p>,
  },
  {
    id: 'analytics',
  ...
  {
    id: 'laws-flags',
    title: 'The ⚖️ flags/laws icon',
    group: 'Everywhere',
    keywords: 'laws flags rules violations panel scale icon',
    body: <p className="text-sm text-gray-700">Opens a panel of standing flags/rules and outstanding violations relevant to whichever tab you’re on (e.g. items with negative stock, missing prices) — a running checklist rather than a one-off popup.</p>,
  },
  {
    id: 'large-screen',
    ...
    body: (
      <div className="space-y-1.5">
        {[
          ...
          ['GMC', 'Grony Multimedia as Customer — stock taken for the shop’s own internal use, not a real sale.'],
...
export function TrainingGuideModal({ isOpen, onClose }: {
  ...
  // Explicitly picked topic (survives a search that no longer matches it --
  // searching is for finding something new, not for hiding what's open).
  const picked = activeId ? TOPICS.find(t => t.id === activeId) ?? null : null
  // What the content pane actually shows -- picked, or the first filtered
  // topic as a sane desktop default when nothing's been picked yet.
  const active = picked ?? filtered[0] ?? null
  // Phone only: is a topic open (detail view) or should the list show?
