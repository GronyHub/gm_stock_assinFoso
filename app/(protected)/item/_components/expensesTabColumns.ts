...
//
// Account/Date are deliberately NOT in here -- they're frozen columns in
// ExpenseTable (always rendered first, sticky while scrolling right, own
// dedicated <td>s), same as Sales/Bills keep their own pinned columns out
// of their ColKey unions. Listing them here made the picker offer show/
// hide, reorder, and rename controls that had no effect on the table.
// Group has no such reason to be frozen -- it's a normal column here.
//
// force-hidden while grouped by that field. Property columns are shown
// only when viewing properties.
export type ColKey = 'group' | 'is_property' | 'amount' | 'expense_type' | 'vendor' | 'source' | 'by' | 'is_related_expense' | 'related_property' | 'related_reasons' | 'property_status' | 'property_type' | 'availability' | 'working' | 'location' | 'reason'
