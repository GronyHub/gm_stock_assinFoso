...
// This staff member's own itemized activity for a given day (Time/Activity/
// Dur/Total, same shape as Home's own feed table), backed by
// /api/staff-times/worked-detail so the numbers here always match what
// PresentStaffBar's own chip is summarizing. Defaults to today; the ◀/▶
// arrows step through previous days.
//
// The "Times" tab on a staff member's own personal page
// (StaffMemberPersonalTab) -- used to be its own standalone modal
// (StaffTimeDetailModal), opened straight from PresentStaffBar. Now that
  ...
  return (
    <div className="flex flex-col">
      ...
      {loading ? (
        ...
        <>
          <div className="px-1 py-2 border-b border-gray-100 flex items-center justify-between text-[10px] text-gray-500 shrink-0">
            <span>
              ...
              <table className="w-full border-collapse">
                <thead className="sticky top-0 bg-white">
                  <tr className="text-[7px] font-semibold text-gray-400 uppercase tracking-wide border-b border-gray-200">
                    <th className="text-left pl-1 pr-1 py-1 whitespace-nowrap">Time</th>
                    <th className="text-left px-1 py-1">Activity</th>
                    <th className="text-left px-1 py-1 whitespace-nowrap">Dur</th>
                    <th className="text-left px-1 pr-1 py-1 whitespace-nowrap">Total</th>
                  </tr>
                </thead>
                <tbody>
                  {detail.activity.map(a => (
                    <tr key={a.id} className="hover:bg-gray-50">
                      <td className="pl-1 pr-1 py-0.5 text-[8px] text-gray-400 whitespace-nowrap">{fmtClockTime(a.created_at)}</td>
                      ...
                      <td className="px-1 pr-1 py-0.5 text-[8px] text-gray-500 font-semibold whitespace-nowrap">{formatDuration(a.running_total_seconds)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
