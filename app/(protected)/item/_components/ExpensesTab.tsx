...
export default function ExpensesTab({ search, onFlagCountChange }: Props) {
  ...
  const viewButtons: { key: string; letter: string; label: string; description: string; active: boolean; onChange: () => void }[] = [
    ...
    { key: 'by_account', letter: 'A', label: 'By Account', description: 'Group expenses by their account category to see totals and records for each account.', active: groupBy === 'account', onChange: () => { setActiveFlag(null); setGroupBy(g => g === 'account' ? 'none' : 'account') } },
    { key: 'by_vendor', letter: 'V', label: 'By Vendor', description: 'Group expenses by vendor to see total spending and records for each supplier.', active: groupBy === 'vendor', onChange: () => { setActiveFlag(null); setGroupBy(g => g === 'vendor' ? 'none' : 'vendor') } },
    { key: 'show_properties', letter: 'P', label: 'All Properties', description: 'View only expenses marked as properties/equipment.', active: showProperties && !showNonProperties, onChange: () => { setActiveFlag(null); setPropertyAvailabilityFilter('all'); setPropertyTypeFilter(null); if (showProperties && !showNonProperties) { setShowProperties(true); setShowNonProperties(true) } else { setShowProperties(true); setShowNonProperties(false) } } },
    { key: 'show_non_properties', letter: 'N', label: 'Non-Properties', description: 'View only regular expenses (non-property expenditures).', active: !showProperties && showNonProperties, onChange: () => { setActiveFlag(null); if (!showProperties && showNonProperties) { setShowProperties(true); setShowNonProperties(true) } else { setShowProperties(false); setShowNonProperties(true) } } },
    { key: 'prop_available', letter: 'A', label: 'Properties Available', description: 'View only properties currently at a Grony shop location (marked as available).', active: propertyAvailabilityFilter === 'available', onChange: () => { setActiveFlag(null); setPropertyTypeFilter(null); setPropertyAvailabilityFilter(f => f === 'available' ? 'all' : 'available') } },
    { key: 'prop_not_available', letter: 'U', label: 'Properties Not Available', description: 'View only properties that are currently away from shop (spoilt, stolen, or at Grony\'s house).', active: propertyAvailabilityFilter === 'not_available', onChange: () => { setActiveFlag(null); setPropertyTypeFilter(null); setPropertyAvailabilityFilter(f => f === 'not_available' ? 'all' : 'not_available') } },
    { key: 'printers', letter: 'M', label: 'Printers', description: 'View only printers and printer-related expenses.', active: propertyTypeFilter === 'Printer', onChange: () => { setActiveFlag(null); setPropertyAvailabilityFilter('all'); setPropertyTypeFilter(t => t === 'Printer' ? null : 'Printer') } },
  ]
