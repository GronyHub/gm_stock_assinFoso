...
export default function ItemDetailPanel({ itemId, collapsed, onExpand }: ItemDetailPanelProps) {
  ...
  return (
    <div className="overflow-x-auto">
      <div className="px-1 py-1 bg-white border-b border-gray-200 flex items-center justify-between">
        <p className="text-sm font-bold text-red-600 truncate">{item.item_name}</p>
        {collapsed && !expanded && (
          <button onClick={handleExpand}
            className="text-[7px] font-semibold text-blue-600 hover:underline whitespace-nowrap ml-2">
            Expand ▼
          </button>
        )}
      </div>
      {showFilters && (
        <div className="flex items-center gap-1 pb-0.5 flex-wrap">
          <button onClick={() => setShowPrices(p => !p)}
            ...
            className={`shrink-0 text-[7px] font-semibold px-1.5 py-0 rounded-lg whitespace-nowrap transition
