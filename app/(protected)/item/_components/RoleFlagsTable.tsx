...
export function RoleFlagsTable({ violations, assignments, deadlines, assignedBy, assignedOn, vSettings, onGoToViolation }: {
      ...
      <table className="w-full text-[11px] border-collapse">
        ...
        <tbody className="divide-y divide-gray-100">
          {violations.map(v => {
            ...
            return (
              <tr key={v.type} onClick={() => onGoToViolation?.(violationKey)}
                className={`cursor-pointer hover:bg-blue-50 transition ${atRisk ? 'bg-red-50' : ''}`}>
