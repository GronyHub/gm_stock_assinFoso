...
export async function POST() {
  ...
  for (const id of IDS) {
    try {
      ...
      await sql`DELETE FROM good_service_matches WHERE LOWER(TRIM(good_name)) = LOWER(TRIM(${item.canonical_name})) OR LOWER(TRIM(service_name)) = LOWER(TRIM(${item.canonical_name}))`.catch(() => {})
      await sql`DELETE FROM item_audio_advert_status WHERE item_id = ${id}`.catch(() => {})
      await sql`DELETE FROM pack_tradeoffs WHERE item_id = ${id}`.catch(() => {})
