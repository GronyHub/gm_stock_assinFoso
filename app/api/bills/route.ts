...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // 10 minutes flat -- entering a bill has no per-line time estimate like a
    // live sale tap does, but typing one in by hand is real work, same
    // treatment every other manual-entry form gets (expenses, purchase
    // orders, vendors, customers).
    await logActivity(enteredBy ?? 'Unknown', 'added bill', `${lines.length} line${lines.length > 1 ? 's' : ''} · ₵${grandTotal.toFixed(2)}${vendorNote}`, 600)
