...
export async function GET() {
  ...
  `
  const sources = await sql`SELECT id, canonical_name, zoho_item_id, zoho_item_name, product_type, status, source, is_legacy FROM items WHERE source != 'zoho_historical' LIMIT 5`
