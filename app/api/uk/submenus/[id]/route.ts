...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [row] = await sql`
    UPDATE uk_submenus SET name = ${text} WHERE id = ${Number(id)}
...
// Columns cascade-delete with their submenu -- see uk_columns' FK.
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const session = await auth()
  if (!session || !isAllowed(session)) return NextResponse.json({ error: 'Forbidden' }, { status: 403 })

  const { id } = await params
  await sql`DELETE FROM uk_submenus WHERE id = ${Number(id)}`
