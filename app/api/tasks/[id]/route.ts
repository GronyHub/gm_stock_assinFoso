...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const recurrence_type   = has('recurrence_type') ? body.recurrence_type : current.recurrence_type
  const recurrence_days   = has('recurrence_days') ? body.recurrence_days : current.recurrence_days
  ...
  const [row] = await sql`
    UPDATE custom_tasks SET
      ...
      task_type = ${task_type || 'General +'}, assigned_to = ${assigned_to || null},
      recurrence_type = ${recurrence_type || null}, recurrence_days = ${recurrence_days ? JSON.stringify(recurrence_days) : null}
    WHERE id = ${taskId}
