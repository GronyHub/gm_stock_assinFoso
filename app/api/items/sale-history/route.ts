...
//
// This sums every sale line ever recorded, for every item, and was running
// fresh on every single Item hub page load. A multi-year average barely
// moves within a couple of hours, so it's cached the same way the app's
// other slow-changing summaries already are (see /api/items/all).
export async function GET() {
  const { error } = await requireAuth()
  if (error) return error

  try {
    const result = await getCached('items:sale-history', 7200, async () => {
